...
const Home = (props) => {
    ...
    useEffect(() => {
        ...
        xhrUpcoming.onreadystatechange = ()=>{

            if(xhrUpcoming.readyState === 4)
            {
                setUpcomingMovies(JSON.parse(xhrUpcoming.responseText).movies);
            }
        };
        ...
        xhrReleased.onreadystatechange = ()=>{
    ...
    const filterApplyHandler = () => {
        ...
        if (genres.length > 0) {
            queryString += "&genre=" + encodeURIComponent(genres.toString());
        }
        if (artists.length > 0) {
            queryString += "&artists=" + encodeURIComponent(artists.toString());
        }
        if (releaseDateStart !== "") {
